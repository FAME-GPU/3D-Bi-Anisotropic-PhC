function [EV,EW,nit] = Lanczos_gpu(dfun_A,n,eigen_wanted,option)
    ...
    norm_dstart_vec   = norm(dstart_vec,2);
    dstart_vec(1:n,1) = dstart_vec / norm_dstart_vec;
    
    dQ(1:n,1) = dstart_vec(1:n,1);
    [dQ, dalpha, dbeta] = Lanczos_step_gpu(dfun_A, n, dQ, dalpha, dbeta, 0, k, dtmp(1:n), dtmp(n+1:2*n), dtmp(2*n+1:3*n), dtmp(3*n+1:4*n), dtmp(4*n+1:5*n));
    while nit < itmax
        nit = nit + 1;
        % expend Krylov subspace by Lanczos
        [dQ, dalpha, dbeta] = Lanczos_step_gpu(dfun_A, n, dQ, dalpha, dbeta, k, m, dtmp(1:n), dtmp(n+1:2*n), dtmp(2*n+1:3*n), dtmp(3*n+1:4*n), dtmp(4*n+1:5*n));
        % restart step
...
%% Lanczos Step
function [dQ, dalpha, dbeta] = Lanczos_step_gpu(dfun_A, n, dQ, dalpha, dbeta, k, m, dw, dr, dtmp_n_1, dtmp_n_2, dtmp_n_3)
    for j = k+1 : m
        dw(1:n,1) = dfun_A(dQ(:,j));
        dalpha(j) = real( dQ(:,j)'*dw );
        if j == 1
            dr(1:n,1) = dw(1:n,1) - dalpha(j)*dQ(1:n,j);
        else
            dr(1:n,1) = dw(1:n,1) - dalpha(j)*dQ(1:n,j) - dbeta(j-1)*dQ(1:n,j-1);
        end
        [dQ(1:n,j+1), dbeta(j)] = gsreorthog_gpu(dQ(1:n,1:j) , dr(1:n,1), n, j, dtmp_n_1, dtmp_n_2, dtmp_n_3 );
    end
end
...
%% Reorthogonalize 
function [dp, tho] = gsreorthog_gpu(dQ, dx, m ,n, dp, dr, dx_bar)
