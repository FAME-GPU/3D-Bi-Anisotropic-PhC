function [ Point_idx, Point_idx_aircube ] = Material_Locate_Domain_Decomposition( Point_set, lattice_constant, woodpile_width, airwall_thickness, periodic_num, edge_len )
    Point_idx = 1:size(Point_set,1);
    
    Point_idx_airpile_x = Material_Locate_Airpile( Point_set+0.5*edge_len, lattice_constant, woodpile_width, airwall_thickness, periodic_num(2), periodic_num(3), 2, 3 );    
    Point_idx_airpile_y = Material_Locate_Airpile( Point_set+0.5*edge_len, lattice_constant, woodpile_width, airwall_thickness, periodic_num(3), periodic_num(1), 3, 1 );
    Point_idx_airpile_z = Material_Locate_Airpile( Point_set+0.5*edge_len, lattice_constant, woodpile_width, airwall_thickness, periodic_num(1), periodic_num(2), 1, 2 );
    Point_idx_airwall   = find( (Point_set(:,1) < -0.5*edge_len(1)+airwall_thickness(1) ) | (Point_set(:,1) > 0.5*edge_len(1)-airwall_thickness(1) ) | ...
                                (Point_set(:,2) < -0.5*edge_len(1)+airwall_thickness(2) ) | (Point_set(:,2) > 0.5*edge_len(2)-airwall_thickness(2) ) | ...
                                (Point_set(:,3) < -0.5*edge_len(1)+airwall_thickness(3) ) | (Point_set(:,3) > 0.5*edge_len(3)-airwall_thickness(3) ) );
    Point_idx_aircube = intersect(Point_idx_airpile_x,Point_idx_airpile_y);
    Point_idx_aircube = intersect(Point_idx_aircube,Point_idx_airpile_z);
    
    Point_idx = setdiff(Point_idx, Point_idx_aircube);
